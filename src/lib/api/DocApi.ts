...
import RankingItem from '../model/RankingItem'
...
const getRanking = async (count = 10): Promise<RankingItem[]> => {
  ...
  const ref = db.collection('ranking').orderBy('score', 'desc').limit(count)
  const docs = await ref.get()
  docs.forEach(doc => {
    const data = doc.data()
    if (!data) { return }
    const { uid, name, score, created } = data
...
const getRank = async (score: number, maxRank = 10, shouldRefresh = false): Promise<number> => {
  ...
  return rank === (cachedRank.length + 1) ? 0 : rank
}
